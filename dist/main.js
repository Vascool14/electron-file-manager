...
const conversions = __webpack_require__(/*! ./conversions */ "./node_modules/slice-ansi/node_modules/color-convert/conversions.js");
...
const electron_context_menu_1 = __importDefault(__webpack_require__(/*! electron-context-menu */ "./node_modules/electron-context-menu/index.js"));
...
const createWindow = () => {
    ...
    mainWindow.on("close", function (e) {
        if (mainWindow) {
            ...
            let response = electron_1.dialog.showMessageBoxSync(mainWindow, {
                type: "question",
                buttons: ["Yes", "No"],
                title: "Confirm",
                message: message,
...
//working
electron_1.ipcMain.handle("read-initial-data", async () => {
    const userDataPath = electron_1.app.getPath("userData");
    const filePath = path.join(userDataPath, "savedData.json");
    if (filePath)
        try {
...
electron_1.ipcMain.handle("open-file-dialog", async (event) => {
    const result = await electron_1.dialog.showOpenDialog({
        properties: ["openFile"],
    });
    return result;
});
electron_1.ipcMain.handle("open-directory-dialog", async (event) => {
    const result = await electron_1.dialog.showOpenDialog({
        properties: ["openDirectory"],
    });
    return result;
});
electron_1.ipcMain.on("open-in-explorer", (_event, filePath) => {
    electron_1.shell.showItemInFolder(filePath);
    // shell.openPath - in order to open the file also -> do a checkbox in case user want folder or file
    // shell.openPath(filePath);
});
electron_1.ipcMain.on("save-data", (_event, data) => {
    const userDataPath = electron_1.app.getPath("userData");
    const filePath = path.join(userDataPath, "savedData.json");
